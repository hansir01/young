	...
	<!----------------第一列 start-------------------->
		...
		<div class="dc">
			<div class="hottype">
				...
				<div class="typelist">
					<a href="/">全部</a>
					<?php if(is_array($type_data)): foreach($type_data as $key=>$vo): ?><a href="/index.php?typeid=<?php echo ($vo["id"]); ?>"><?php echo ($vo["typename"]); ?></a><?php endforeach; endif; ?>
				</div>
			</div>
		</div>
			...
			success:function(data)
			{
				if(data.status == 1)
				{
					$.each(data.list,function(i,pic)
					{
						...
						str+='<a class="hot_add" id="'+pic.id+'" href="/index.php/index/detail?id='+pic.id+'" title="'+pic.title+'" target="_blank">'
						str+='<img src="/Uploads'+pic.pic_url+'" height="'+pic.height+'" alt="'+pic.title+'">'
						str+='</a>'
						str+='</div>'
						str+='<div class="onedes">'+pic.title+'</div><div class="onetit">眼缘：12</div>'
						str+='</div>'
						for(var i=0;i<4;i++)
						{
